...
function Install_ct() {
  ...
  if [[ ${addyn} == [Yy] ]]; then
    ...
    mv gost /usr/bin/gost
    chmod -R 777 /usr/bin/gost
    wget --no-check-certificate https://raw.githubusercontent.com/siemenstutorials/master/gost.service && chmod -R 777 gost.service && mv gost.service /usr/lib/systemd/system
    mkdir /etc/gost && wget --no-check-certificate https://raw.githubusercontent.com/siemenstutorials/master/config.json && mv config.json /etc/gost && chmod -R 777 /etc/gost
  fi
  ...
  if test -a /usr/bin/gost -a /usr/lib/systemctl/gost.service -a /etc/gost/config.json; then
    ...
    echo "Gost若没有安装成功，请提交issue"
    rm -rf "$(pwd)"/gost
    rm -rf "$(pwd)"/gost.service
    rm -rf "$(pwd)"/config.json
    rm -rf "$(pwd)"/Gost_OneKey.sh
  fi
}
...
function read_protocol() {
  echo -e "请选择Gost模式: "
  echo -e "-----------------------------------"
  echo -e "[1] tcp+udp不加密流量转发"
  echo -e "说明: 一般用在国内中转机上"
  echo -e "-----------------------------------"
  echo -e "[2] 加密隧道流量转发"
  echo -e "说明: (1)选择此协议须配合落地机器安装Gost并配置协议[3]进行对接使用"
  echo -e "      (2)一般用在国内中转机上"
  echo -e "-----------------------------------"
  echo -e "[3] 解密由gost传输而来的流量并转发"
  echo -e "说明: 一般用于国外落地机器，用于接收国内中转机的加密流量"
  echo -e "-----------------------------------"
...
update_sh() {
  ol_version=$(curl -L -s https://raw.githubusercontent.com/siemenstutorials/master/Gost_OneKey.sh | grep "shell_version=" | head -1 | awk -F '=|"' '{print $3}')
  if [[ "$shell_version" != "$ol_version" ]]; then
    echo -e "存在新版本，是否更新 [Y/N]?"
    read -r update_confirm
    case $update_confirm in
    [yY][eE][sS] | [yY])
      wget -N --no-check-certificate https://raw.githubusercontent.com/siemenstutorials/master/Gost_OneKey.sh
      echo -e "更新完成"
      exit 0
      ;;
    *) ;;
...
update_sh
echo && echo -e "                 GostOneKey安装配置脚本"${Red_font_prefix}[${shell_version}]${Font_color_suffix}"
  ----------- SiemensTutorials -----------
